...
def role_attention_summary(df: pd.DataFrame, attention_rows: pd.DataFrame):
    """Optional attention flags aggregated by structural role."""
    columns = ["role", "flag", "n_nodes", "share_role", "avg_priority_score",
               "top_gids", "meaning"]
    if attention_rows.empty:
        return pd.DataFrame(columns=columns)
    role_sizes = df.groupby("role").size().to_dict()
    rows = []
    for (role, flag), group in attention_rows.groupby(["role", "flag"], sort=True):
        leaders = group.sort_values(["priority_score", "gid"], ascending=[False, True]).head(5)
        n_nodes = group.gid.nunique()
        rows.append({"role": role,
                     "flag": flag,
                     "n_nodes": int(n_nodes),
                     "share_role": round(n_nodes / role_sizes[role], 4),
                     "avg_priority_score": round(float(group.priority_score.mean()), 6),
                     "top_gids": ";".join(str(int(x)) for x in leaders.gid),
                     "meaning": group.meaning.iloc[0]})
    result = pd.DataFrame(rows, columns=columns)
    result["_role_order"] = result.role.map({role: idx for idx, role in enumerate(ROLE_ORDER)})
    return result.sort_values(
        ["_role_order", "n_nodes", "flag"], ascending=[True, False, True]
...
def component_role_matrix(graph: nx.DiGraph, df: pd.DataFrame):
    """Role composition inside each weakly connected component."""
    columns = ["component_id", "role", "n_nodes", "share_component",
               "n_seed", "boundary_nodes", "avg_priority_score", "top_gids"]
    if df.empty:
        return pd.DataFrame(columns=columns)
    components = sorted(nx.weakly_connected_components(graph), key=lambda c: (-len(c), min(c)))
    component_id = {int(gid): idx for idx, members in enumerate(components, start=1) for gid in members}
    ...
    component_sizes = work.groupby("component_id").size().to_dict()
    rows = []
    for (cid, role), group in work.groupby(["component_id", "role"], sort=True):
        leaders = group.sort_values(["priority_score", "gid"], ascending=[False, True]).head(5)
        rows.append({"component_id": int(cid),
                     "role": role,
                     "n_nodes": int(len(group)),
                     "share_component": round(len(group) / component_sizes[cid], 4),
                     "n_seed": int(group.is_seed.sum()),
                     "boundary_nodes": int(group.boundary.sum()),
                     "avg_priority_score": round(float(group.priority_score.mean()), 6),
                     "top_gids": ";".join(str(x) for x in leaders.gid)})
    return pd.DataFrame(rows, columns=columns).sort_values(
        ["component_id", "n_nodes", "role"], ascending=[True, False, True]
    ).reset_index(drop=True)
...
def cluster_depth_matrix(df: pd.DataFrame):
    """Depth composition for each detected cluster."""
    columns = ["cluster_id", "depth", "n_nodes", "share_cluster", "n_seed",
               "boundary_nodes", "avg_priority_score", "top_gids"]
    if df.empty:
        return pd.DataFrame(columns=columns)
    rows = []
    cluster_sizes = df.groupby("cluster_id").size().to_dict()
    for (cluster_id, depth), group in df.groupby(["cluster_id", "depth"], sort=True):
        leaders = group.sort_values(["priority_score", "gid"], ascending=[False, True]).head(5)
        rows.append({"cluster_id": int(cluster_id),
                     "depth": int(depth),
                     "n_nodes": int(len(group)),
...
def top_counterparties(edges: pd.DataFrame, df: pd.DataFrame, per_node: int = 3):
    ...
    columns = ["gid", "direction", "rank", "counterparty_gid", "sum_kzt", "n_tx",
               "counterparty_role", "counterparty_cluster", "counterparty_seed"]
    if edges.empty:
        return pd.DataFrame(columns=columns)
    meta = df.set_index("gid")
    rows = []
    ...
    for direction, owner_col, counterparty_col in directions:
        ranked = edges.sort_values([owner_col, "sum_kzt", counterparty_col],
                                   ascending=[True, False, True])
        for gid, group in ranked.groupby(owner_col, sort=True):
            for rank, edge in enumerate(group.head(per_node).itertuples(index=False), start=1):
                counterparty = int(getattr(edge, counterparty_col))
                row = meta.loc[counterparty]
                rows.append({"gid": int(gid),
                             "direction": direction,
                             "rank": rank,
                             "counterparty_gid": counterparty,
                             "sum_kzt": round(float(edge.sum_kzt), 2),
                             "n_tx": int(edge.n_tx),
                             "counterparty_role": row.role,
                             "counterparty_cluster": int(row.cluster_id),
                             "counterparty_seed": bool(row.is_seed)})
    return pd.DataFrame(rows, columns=columns).sort_values(
        ["gid", "direction", "rank", "counterparty_gid"]
    ).reset_index(drop=True)
